...
**Этот режим достаёт дальше назад, чем другой.** Рантайм просит Dart `^3.10.0` и Flutter
`>=3.38.0`; генератор и плагин линтера — `^3.13.0`, потому что им нужен analyzer 13. Значит
приложение, оставшееся на Flutter 3.38, может начать отсюда и взять
[GUIDE_CODEGEN.ru.md](GUIDE_CODEGEN.ru.md) после апгрейда — почему при этом ничего не выбрасывается,
см. [§15](#15-когда-добавлять-генератор).
