...
Галерея написана на английском, русском и китайском, язык переключается прямо на хабе — и вместе с
ней каждый экран, который она монтирует. У каждого пакета-примера свои `l10n/*.arb` и свой делегат,
которые галерея собирает рядом со своим и с инспекторским: так и выглядит настоящее многопакетное
Flutter-приложение. В двух местах проза лежала ниже виджетов, где нет `BuildContext` и спросить про
язык не у кого, — теперь оба сообщают факты, а называет их экран.
...
поставляет двенадцать warning-правил, все построены на том же слое разбора `alloy_analyzer`, что и
