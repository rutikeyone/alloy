...
Каждый рантайм-пакет требует Dart `^3.10.0` и Flutter `>=3.38.0`. Три инструментальных —
`alloy_analyzer`, `alloy_generator`, `alloy_lint` — требуют Dart `^3.13.0`, и это разделение
измерено, а не выбрано из осторожности: `alloy_lint` не компилируется без analyzer 13, а ниже
Dart 3.11 парсер перестаёт видеть `@AlloyParam` на параметрах конструктора — это хуже, чем отказ
собираться. analyzer 13 тянет `_fe_analyzer_shared 100`, которому нужен Dart 3.11.

Значит приложение, оставшееся на Flutter 3.38, может взять Manual Mode сегодня и получить генератор
после апгрейда, ничего не выбрасывая: сгенерированный контейнер — такой же `AlloyScopeBuilder`, как
написанные руками.

CI гоняет `stable` и `beta`, а не матрицу прошлых релизов, плюс отдельную job на Flutter 3.38.9,
которая резолвит, анализирует и тестирует каждый пакет против его собственного пола
(`tool/floor_check.sh`). Пол, который никто не проверяет, — утверждение, которое протухнет.
