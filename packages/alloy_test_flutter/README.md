...
Widget-test helpers for [Alloy](https://github.com/rutikeyone/alloy). Add it as a `dev_dependency`.
...
Two pumps rather than one, because `AlloyScopeWidget` and `AlloyAppScope` publish the scope only
...
`AlloyScopeProvider.of` searches *above* the element it is given and would skip the very provider
that was found.

## Why this is not in `alloy_test`

`alloy_test` is pure Dart on `test_api` and `matcher`, so its graph helpers work under `dart test`
...
The same reasoning splits `alloy` from `alloy_flutter`.
