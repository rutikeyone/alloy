      ...
      () async {
        ...
        final keyed = {
          for (final entry in talker.history)
            if (entry.key?.startsWith('cobalt-') ?? false) entry.key!,
        };
        final titled = {
          for (final entry in talker.history)
            if (entry.title?.startsWith('cobalt-') ?? false) entry.title!,
        };
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: CobaltInspectorTheme(
          data: palette,
          child: CobaltTalkerScreen(talker: Talker()),
        ),
      ),
    );
    await tester.pumpAndSettle();

    expect(find.byType(CobaltTalkerScreen), findsOneWidget);
  });
}
