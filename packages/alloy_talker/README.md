...
[Alloy](https://github.com/rutikeyone/alloy). The DI graph reports itself as typed talker logs.
...
The other adapters in this family implement `AlloyLogSink` — they receive a formatted string. This
one implements `AlloyObserver` directly, because that is what talker is for: each kind of event
becomes its own `TalkerLog` with a title and a colour, and `TalkerScreen` can then filter them
apart.

| Title | What lands there |
|---|---|
| `alloy-scope` | a scope was pushed, is disposing, or is gone |
| `alloy-startup` | bootstrap steps and async initialization |
| `alloy-instance` | an instance was built or released — off unless `verbose` |
| `alloy-failure` | init failed, teardown could not release something, a bootstrap step broke |

## `verbose` is off on purpose

Per-instance logs are the loudest thing Alloy can emit: a real graph builds a great many, and the
...
An exception thrown from inside an observer is swallowed by Alloy — watching cannot break what it
