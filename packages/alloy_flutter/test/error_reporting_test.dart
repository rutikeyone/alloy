...
final class StubbornScope implements AlloyScopeBuilder {
  const StubbornScope();

  @override
  void build(AlloyScope scope) => scope.registerSingleton<Stubborn>(Stubborn());
}
...
void main() {
  ...
  ///
  /// `AlloyScopeWidget` has a third report, for an `init()` that fails after
  ...
  group('a failure with nobody left to show it', () {
    testWidgets('a teardown that throws on unmount is reported', (
      ...
      expect(reported.single.exception, isA<AlloyDisposeError>());
      expect(reported.single.context.toString(), contains('session'));
    });

    testWidgets('AlloyAppScope reports a root it could not release', (
      tester,
    ) async {
      capture();
      await tester.pumpWidget(
      ...
      expect(
        ...
        contains('the root scope owned by AlloyAppScope'),
      );
    });
  });
}
