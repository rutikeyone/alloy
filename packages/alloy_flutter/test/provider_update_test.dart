...
AlloyScope scopeSaying(String text) =>
    alloyTestRoot(name: 'app')..registerSingleton<Label>(Label(text));
...
class _HostState extends State<_Host> {
  late AlloyScope _scope = widget.first;

  void swap(AlloyScope next) => setState(() => _scope = next);

  @override
  Widget build(BuildContext context) =>
      AlloyScopeProvider(scope: _scope, child: widget.child);
}
...
class _Reader extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final text = context.alloy<Label>().text;
    builds.add(text);
