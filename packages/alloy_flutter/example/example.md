# alloy_flutter example

`AlloyAppScope` owns the root scope for as long as the app is mounted. Its
...
class HomeScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) =>
      Scaffold(body: Center(child: Text('${context.alloy<Database>()}')));
}
```

A scope that lives as long as one screen is `AlloyScopeWidget`; a full app with
