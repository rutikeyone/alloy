...
import 'package:cobalt_inspector/src/record_detail_sheet.dart';
...
class _EventLogViewState extends State<EventLogView> {
  ...
  Iterable<CobaltLogEntry> _matching(List<CobaltLogEntry> entries) sync* {
    for (final entry in entries) {
      final record = entry.record;
      if (_family != null && CobaltInspectorFamily.of(record.kind) != _family) {
        continue;
      }
      if (_query.isEmpty) {
        yield entry;
  ...
  static Map<CobaltInspectorFamily, int> _counts(List<CobaltLogEntry> entries) {
    final counts = {for (final f in CobaltInspectorFamily.values) f: 0};
    for (final entry in entries) {
      final family = CobaltInspectorFamily.of(entry.record.kind);
...
class _FamilyFilter extends StatelessWidget {
  ...
  final CobaltInspectorFamily? selected;
  final CobaltInspectorThemeData theme;
  final CobaltInspectorL10n strings;
  final Map<CobaltInspectorFamily, int> counts;
  final ValueChanged<CobaltInspectorFamily?> onChanged;
