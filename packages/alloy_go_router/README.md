...
go_router bindings for [Alloy](https://github.com/rutikeyone/alloy). A scope whose lifetime is a
...
Inside the flow nothing new applies: `context.alloy<T>()` resolves from the nearest scope, so the
...
- **A frame of `loading` on every rebuild.** `AlloyScopeWidget` publishes its scope only after
...
- **Nested flows cost a frame each.** A flow inside a flow publishes its scope one frame after its
  parent does.
- **Registrations are built during the widget build phase.** `AlloyScopeBuilder.build` runs inside
...
- **A scope cannot be reparented.** If the root scope is replaced, the subtree has to be rebuilt
  rather than left pointing at the old root. `AlloyAppScope` handles this — it keys the provider it
