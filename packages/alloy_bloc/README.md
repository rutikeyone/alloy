...
Lets an [Alloy](https://github.com/rutikeyone/alloy) scope close the blocs it built.
...
`alloy_lint` reports it as `alloy_registration_is_never_released`, which is how you find the ones
...
Alloy builds the bloc; `BlocBuilder` still renders it. Resolve it where you would have created it,
...
[`alloy_talker`](https://pub.dev/packages/alloy_talker) makes against `alloy_talker_flutter`. There
